...
pub async fn test_gm_action() {
    let result = gm_action(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_get_player_token() {
    let result = get_player_token(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_get_game() {
    let result = get_game(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_post_json_orders() {
    let result = post_json_orders(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_get_orders() {
    let result = get_orders(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_set_ready() {
    let result = set_ready(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_get_ready() {
    let result = get_ready(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}

#[tokio::test]
pub async fn test_get_token_access_log() {
    let result = get_token_access_log(DEFAULT_URL.clone()).await;
    let _ = dbg!(result);
}